...
describe('connpass-event-search', () => {
  ...
  describe('スプレッドシート列定義のテスト', () => {
    it('設定シートの列定義が正しい', () => {
      expect(SPREADSHEET_COLUMNS.KEYWORDS).toBe(1);
      expect(SPREADSHEET_COLUMNS.CONNPASS_API_KEY).toBe(2);
      expect(SPREADSHEET_COLUMNS.LINE_CHANNEL_ACCESS_TOKEN).toBe(3);
    });
